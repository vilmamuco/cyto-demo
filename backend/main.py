...
def get_targetNodes():
    ...
    return jsonify(
        [
            {"id": node["data"]["id"], "label": node["data"]["label"]}
...
def update_Position():
    ...
    nodes.update_one({"data.id": id}, {"$set": {"position.x": x, "position.y": y}})
...
def update_Color():
    ...
    nodes.update_one({"data.id": id}, {"$set": {"style.background-color": color}})
    # maybe return just ok
    return get_elements()
...
def save_newElement():
    ...
    if (nodes.find() is None) or nodes.count_documents({"data.id": id}) == 0:
        ...
        if target:
            edges.insert_one(
                {"data": {"source": id, "target": target, "label": "test add"}}
            )
    return get_elements()
...
def update_element():
    ...
    if (nodes.find() is not None) and nodes.count_documents({"data.id": id}) > 0:
        nodes.update_one(
            {"data.id": id},
            {"$set": {"data.label": label, "style.background-color": color}},
        )
        if (
            target
            and edges.count_documents(
                {"$and": [{"data.target": target}, {"data.source": id}]}
            )
            == 0
        ):
            edges.insert_one(
...
def add_edge():
    ...
    if (edges.find() is None) or (
        edges.find() is not None
        and edges.count_documents(
            {"$and": [{"data.target": target}, {"data.source": source}]}
        )
        == 0
    ):
        edges.insert_one(
            {"data": {"source": source, "target": target, "label": "test add"}}
        )
    return get_elements()
